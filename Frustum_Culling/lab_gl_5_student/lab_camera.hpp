...
namespace lab{
	...
	public:
		...
		void rotateFPSoX(float angle){
			//rotatie
			up = this->up*cos(angle) + this->forward*sin(angle);
			forward = glm::cross(this->up, this->right);
		...
		void rotateFPSoY(float angle){
			//rotatie
			forward = this->forward*cos(angle) + this->right*sin(angle);
			right = glm::cross(this->forward, this->up);
		...
		void rotateFPSoZ(float angle){
			//rotatie
			right = this->right*cos(angle) + this->up*sin(angle);
			up = glm::cross(this->right, this->forward);
