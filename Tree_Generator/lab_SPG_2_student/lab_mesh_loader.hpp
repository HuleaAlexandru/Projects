...
namespace lab{
	...
	class Mesh{
	public:
		Mesh(unsigned int vbo, unsigned int ibo, unsigned int vao, unsigned int count){
			this->vbo = vbo; 
			this->ibo = ibo;
			this->vao = vao;
			this->count = count;
	...
	Mesh* loadObj(const std::string &filename){
		...
		Mesh* m = new Mesh(gl_index_buffer_object,gl_index_buffer_object,gl_vertex_array_object, indices.size());
		return m;
	}
