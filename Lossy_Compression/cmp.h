...
struct block {
	...
	unsigned char index_matrix[BLOCK_SIZE * BLOCK_SIZE] __attribute__((aligned(16)));
...
struct package_t {
	int action_type;	// compress = 0, decompress = 1
	int mod_vect;		// operatii_scalare = 0, operatii_vect = 1, intrinsics = 2
	int mod_dma;		// fara double buffering = 0, cu double buffering = 1
	int num_spus;		// numar spu = 1,2,4,8
	unsigned int nr_blocks;		// numarul de blocuri analizat de spu-ul curent
	int index_block;	// indexul blocului in vectorul de blocuri
	struct img img_pgm;	// imaginea de intrare pentru compresie / de iesire pentru decompresie
	struct c_img img_cmp;	// imaginea de iesire comprimata pentru compresie / imaginea compresata din care se obtine decompresia pentru decompresie
} __attribute__ ((aligned(16)));	
...
void read_pgm(char* path, struct img* in_img){
        ...
        in_img->pixels = (unsigned char*) _alloc(in_img->width * in_img->height * sizeof (unsigned char));
